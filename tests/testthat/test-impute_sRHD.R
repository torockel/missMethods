test_that("impute_sRHD() simple random Hot-Deck imputation", {
  ...
  expect_error(impute_sRHD(data.frame(X = rep(NA, 10), Y = 1:10, Z = rep(NA, 10))),
               "imputation not possible: column\\(s\\) .* are completely missing")

  # check donor_limit errors ------------------------------
  expect_error(impute_sRHD(df_XY_X_miss, donor_limit = 1:4),
               "donor_limit must be of length 1")
  expect_error(impute_sRHD(df_XY_X_miss, donor_limit = -5),
               "donor_limit must be a number >= 1 or the string 'min'")
  expect_error(impute_sRHD(df_XY_X_miss, donor_limit = "asdf"),
               "donor_limit must be a number >= 1 or the string 'min'")
  ...
  expect_condition(impute_sRHD(df_XY_X_miss_one_comp_obs,
                               type = "cols_seq",
                               donor_limit = "min"),
  ...
  expect_error(impute_sRHD(df_XY_X_miss_one_comp_obs, type = "cols_seq",
                           donor_limit = 20),
  ...
  expect_error(impute_sRHD(df_XY_no_comp_obs, type = "sim_comp"),
  ...
  expect_equal(df_XY_X_miss_one_comp_obs_min, df_XY_X_miss_one_comp_obs_99)
  expect_equal(df_XY_X_miss_one_comp_obs_min[, "X"], rep(100, 100))
  expect_error(impute_sRHD(df_XY_X_miss_one_comp_obs, type = "sim_comp",
                           donor_limit = 20),
               "donor_limit = 20 is to low! It must be at least 99")
  ...
  expect_error(impute_sRHD(df_XY_no_comp_obs, type = "sim_part"),
  ...
  expect_equal(impute_sRHD(ds_sim_part_miss1, type = "sim_part"),
               ds_sim_part_correct1)

  ds_sim_part_impossible <- matrix(c(NA, NA, 13,
                                     NA, 22, NA,
                                     31, NA, NA), ncol = 3)
  expect_error(impute_sRHD(ds_sim_part_impossible, type = "sim_part"),
               "there is no appropriate donor for the object\\(s\\)")
  expect_error(impute_sRHD(df_XY_X_miss, type = "sim_part", donor_limit = 2),
              "donor_limit is not implemented for type = sim_part")

  # check special cases -----------------------------------
  expect_error(impute_sRHD(df_one_comp_missing_col),
  ...
  expect_error(impute_sRHD(df_all_NA),
...
test_that("impute_sRHD() works with matrices", {
  ...
  expect_error(impute_sRHD(matrix_100_2_miss, type = "sim_part", donor_limit = 2),
               "donor_limit is not implemented for type = sim_part")
...
test_that("min_donor_limit()", {
  ...
  # check sim_comp ----------------------------------------
  for(i in seq_len(5)) {
    expect_equal(min_donor_limit(M_i_donor[[i]], type = "sim_comp"),
                 correct_i_min_donor_limit[i])
  }

  # check cols_seq ----------------------------------------
  for(i in seq_len(5)) {
    expect_equal(min_donor_limit(M_i_donor[[i]], type = "cols_seq"),
                 correct_i_min_donor_limit[i])
  }
  ...
  # check errors --------------------------
  expect_error(min_donor_limit(matrix(TRUE, ncol = 1), type = "sim_comp"),
               "not possible: there is no completely observed object")
  expect_error(min_donor_limit(matrix(c(TRUE, FALSE), ncol = 2), type = "cols_seq"),
               "not possible: there are completely unobserved variables")
  expect_error(min_donor_limit(M_1_donor, type = "type_not_implemented"),
