    ...
    if (sorting) {
      ...
      if (n_mis > 0) { # to avoid problems with seq and n_mis == 0
        ...
        na_indices <- switch(where,
                            ...
                            upper = ordered_indices[seq(from = nrow(ds) - n_mis + 1,
                                                    to = nrow(ds))],
                            both = c(ordered_indices[seq_len(floor(n_mis / 2))],
                                     ordered_indices[seq(from = nrow(ds) - floor(n_mis / 2) + 1,
                                                         to = nrow(ds))]))
      ...
      if (length(unique(ds[, ctrl_cols[i]])) == 1L) {
        warning("the column ", ctrl_cols[i], " is constant; no missing values ",
...
#'
#' Create missing at random (MAR) values using a censoring mechanism in a data frame or
#' a matrix
#'
#' @template delete
#' @template MAR
#'
#' @details
#' If \code{sorting = TRUE} (the default), the column \code{ctrl_cols[i]} will
#' be sorted.
#' Then the rows with the \code{round(nrow(ds) * p[i])} smallest values will be
#' selected (if \code{where = "lower"} (the default)).
#' Now missing values will be created in the column \code{miss_cols[i]} in
#' these rows.
#' This effectively censors the proportion of \code{p[i]} rows of smallest values
#' in \code{ctrl_cols[i]} in \code{miss_cols[i]}.
#'
#' If \code{where = "upper"}, instead of the rows with the smallest values, the
#' rows with the highest values will be selected.
#' For \code{where = "both"}, the one half of the \code{round(nrow(ds) * p[i])}
#' rows with missing values will be the rows with the smallest values and the
#' other half will be the rows with the highest values.
#' So the censoring rows are dived to the highest and smallest values of
#' \code{ctrl_cols[i]}.
#'
#' If \code{sorting = FALSE}, the rows of \code{ds} will not be sorted.
#' Instead, a quantile will be calculated (using \code{\link[stats]{quantile}}).
#' If \code{where = "lower"}, the \code{quantile(ds[, ctrl_cols[i]], p[i])}
#' will be calculated and all rows with values in \code{ds[, ctrl_cols[i]]}
#' below this quantile will have missing values in \code{miss_cols[i]}.
#' For \code{where = "upper"}, the \code{quantile(ds[, ctrl_cols[i]], 1 - p[i])}
#' will be calculated and all rows with values above this quantile will have
#' missing values.
#' For \code{where = "both"}, the \code{quantile(ds[, ctrl_cols[i]], p[i] / 2)}
#' and \code{quantile(ds[, ctrl_cols[i]], 1 -  p[i] / 2)} will be calculated.
#' All rows with values in \code{ctrl_cols[i]} below the first quantile or
#' above the second quantile will have missing values in \code{miss_cols[i]}.
#'
...
#' below this threshold (see examples).
...
delete_MAR_censoring <- function(ds, p, miss_cols, ctrl_cols, where = "lower", sorting = TRUE) {
  ...
  delete_censoring(ds = ds, p = p, miss_cols = miss_cols, ctrl_cols = ctrl_cols,
...
#'
#' @examples delete_MNAR_censoring(ds, 0.2, "X")
delete_MNAR_censoring <- function(ds, p, miss_cols, where = "lower", sorting = TRUE) {
  ...
  delete_censoring(ds = ds, p = p, miss_cols = miss_cols, ctrl_cols = miss_cols,
                   where = where, sorting = sorting)
}
