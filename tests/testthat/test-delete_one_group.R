test_that("delete_MAR_one_group() calls check_delete_args_MAR()", {
  expect_error(delete_MAR_one_group(df_XY_100, 0.1, 1, ctrl_cols = 3),
...
test_that("delete_one_group() and delete_MAR_one_group() works", {
  ...
  expect_equal(count_NA(delete_MAR_one_group(df_XY_100, 0.001, stochastic = FALSE,
  ...
  # to high p
  expect_warning(df_miss <- delete_MAR_one_group(df_XYZ_100, 0.9, "X", "Y"),
  ...
  df_cutoff_problematic <- data.frame(X = ordered(c(0, 0, rep(1, 6), 2, 2)),
                                      Y = 101:110)
  for(i in 1:20) {
  ...
  # ctrl_col constant
  expect_warning(miss_df <- delete_MAR_one_group(df_XY_X_constant, 0.2,
                                          miss_cols = "Y", ctrl_cols = "X"),
                                     "is constant")
