...
test_that("delete_MAR_censoring() calls check_delete_args_MAR()", {
  expect_error(delete_MAR_censoring(df_XY_100, 0.1, 1, ctrl_cols = 3),
...
test_that("delete_MAR_censoring() and delete_cutoff(), which is called by
  ...
  # ctrl_col constant
  expect_equal(count_NA(delete_MAR_censoring(df_XY_X_constant, 0.2, miss_cols = "Y",
                                    ctrl_cols = "X")),
                 c(X = 0, Y = 4))

  # ctr_col nearly constant
  expect_equal(count_NA(delete_MAR_censoring(df_XY_X_one_outlier, 0.2,
                                          miss_cols = "Y", ctrl_cols = "X")),
               c(X = 0, Y = 4))
  # ordered control column
  expect_equal(count_NA(delete_MAR_censoring(df_with_ord_factors, 0.2,
                                          miss_cols = "Y", ctrl_cols = "X")),
               c(X = 0, Y = 4))
  ...
  expect_equal(count_NA(df_miss), c(X = 30, Y = 0))
  expect_true(all(is.na(df_miss[1:30, "X"])))
  expect_equal(count_NA(delete_MAR_censoring(df_XY_100, 0.001, "X", "Y",
                                          sorting = FALSE)),
               c(X = 1, Y = 0)) # here low p leads to one missing value
  ...
  # check special ctrl_col cases
  # ctrl_col constant
  expect_warning(delete_MAR_censoring(df_XY_X_constant, 0.2, miss_cols = "Y",
...
test_that("delete_MNAR_censoring() works", {
  ...
  expect_error(delete_MNAR_censoring(df_XY_X_miss, 0.1, "X"),
