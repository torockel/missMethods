...
impute_LS_adaptive <- function(ds, k = 10, eps = 1e-6, min_common_obs = 5,
  ...
  ds_for_p_imp_gene <- impute_LS_gene(ds_for_p, k = k, eps = eps, min_common_obs = min_common_obs, return_r_max = TRUE)
  ds_for_p_imp_gene$r_max[M] <- NA # don't use NA values from ds!
  ds_for_p_imp_array <- impute_LS_array(ds_for_p, ds_impute_LS_gene = ds_for_p_imp_gene$imp)
  # calculate errors
  ...
  y_g_list <- impute_LS_gene(ds_mat, k = k, eps = eps, min_common_obs = min_common_obs, return_r_max = TRUE)
  y_a <- impute_LS_array(ds_mat, ds_impute_LS_gene = y_g_list$imp)
  ...
  for (index in which(M)) {
    ...
    if (is.na(r_max)) { # to many missing values in a row to get a r_max (impute_LS_gene imputed row mean!)
      ...
      p <- stats::optimize(e_c, c(0, 1),
                           ...
                           e_g = e_g_all[index_for_mixing], maximum = FALSE)$minimum
