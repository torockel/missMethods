...
test_that("check_delete_args()", {
  ...
  # p -----------------------------------------------------
  expect_error(delete_MCAR(df_XY_100, p = rep(0.1, 3)),
               "length must equal miss_cols")
  expect_error(delete_MCAR(df_XY_100, p = rep(0.1, 2), "X"),
               "length must equal miss_cols")
  expect_error(delete_MCAR(df_XYZ_100, p = rep(0.1, 2), 1:3),
               "length must equal miss_cols")
  expect_error(delete_MCAR(df_XY_100, p = c(1.2, 0.9)),
               "must be between 0 and 1")
  expect_error(delete_MCAR(df_XY_100, p = "a"), "p must be numeric")

  # miss_cols ---------------------------------------------
  expect_error(delete_MCAR(df_XY_100, 0.1, miss_cols = c(2, 3)),
               "indices in miss_cols must")
  expect_error(delete_MCAR(df_XY_100, 0.1, miss_cols = c("X", "Z")),
               "all entries of miss_cols")
  expect_error(delete_MCAR(df_XY_100, 0.1,  c(TRUE, FALSE)),
               "miss_cols must be a vector of column names or indices of ds")
  expect_warning(delete_MCAR(df_XY_100, 0.1,  c(1, 1)),
                 "there are duplicates in miss_cols:")

  # stochastic --------------------------------------------
  expect_error(delete_MCAR(df_XY_100, 0.1, stochastic = "asdf"),
               "stochastic must be logical")
  expect_error(delete_MCAR(df_XY_100, 0.1, stochastic = c(TRUE, TRUE)),
...
test_that("check_delete_args_MCAR() works", {
  ...
  expect_error(delete_MCAR(df_XY_100, c(0.1, 0.2) , p_overall = TRUE),
...
test_that("check_delete_args_MAR() works", {
  ...
  expect_error(delete_MAR_1_to_x(1:5, 0.3, "X", "Y"),
               "ds must be a data.frame or a matrix")

  # ctrl_cols (special errors) ----------------------------
  expect_error(delete_MAR_1_to_x(df_XY_100, 0.1, 2, 1, ctrl_cols = 3),
               "indices in ctrl_cols must be in 1:ncol\\(ds)")
  expect_error(delete_MAR_1_to_x(df_XY_100, 0.1, 2, 1, ctrl_cols = "Z"),
               "all entries of ctrl_cols must be in colnames\\(ds)")
  expect_error(delete_MAR_1_to_x(df_XY_100, 0.1, 2, 1, ctrl_cols = factor("X")),
               "ctrl_cols must be a vector of column names or indices of ds")

  expect_error(delete_MAR_1_to_x(df_XY_X_miss, 0.1, 3, "Y", ctrl_cols = "X"),
               "ctrl_cols must be completely observed; no NAs in ds\\[, ctrl_cols\\] allowed")
  expect_error(delete_MAR_1_to_x(df_XYZ_100, 0.1, 3, miss_cols = "X", ctrl_cols = c("Y", "Z")),
               "length\\(miss_cols) must equal length")

  # special errors for check_delete_args_MAR:
  expect_error(delete_MAR_1_to_x(df_XYZ_100, 0.1, 4, miss_cols = "X", ctrl_cols = "X"),
...
test_that("check_delete_args_MCAR() works", {
  ...
  expect_error(delete_MNAR_1_to_x(1:5, 0.3, "X", "Y"),
               "ds must be a data.frame or a matrix")

  # special errors for check_delete_args_MNAR:
  expect_error(delete_MNAR_1_to_x(df_XY_X_miss, 0.1, 3, "X"),
...
test_that("check_ctrl_cols_1_to_x()", {
  ...
  expect_error(check_ctrl_cols_1_to_x(data.frame(X = letters,
                                                 Y = 1:26,
                                                 Z = LETTERS),
                                      c("X", "Y", "Z")),
...
test_that("find_groups() issues a warning for constant x", {
  expect_warning(find_groups(x = rep(1, 4), cutoff_fun = median, prop = 0.5,
                             use_lpSolve = TRUE,
                             ordered_as_unordered = FALSE),
                 "grouping not possible, because x is constant")

  expect_warning(find_groups(x = factor("a"), cutoff_fun = median, prop = 0.5,
                             use_lpSolve = TRUE,
                             ordered_as_unordered = FALSE),
                 "grouping not possible, because x is constant")
})
...
test_that("find_groups() works in general", {
  expect_equal(find_groups(x = 1:5, cutoff_fun = median, prop = 0.5,
                           use_lpSolve = TRUE,
                           ordered_as_unordered = FALSE),
               list(g1 = 1:2, g2 = 3:5))
...
test_that("find_groups() treats unordered factors correct", {
  expect_equal(find_groups(x = factor(1:5), cutoff_fun = median, prop = 0.5,
                           use_lpSolve = TRUE,
                           ordered_as_unordered = FALSE),
               list(g1 = 1:3, g2 = 4:5))
...
test_that("find_groups() treats ordered factors correct", {
  expect_equal(find_groups(x = ordered(1:5), cutoff_fun = median, prop = 0.5,
                           use_lpSolve = TRUE,
                           ordered_as_unordered = FALSE),
               list(g1 = 1:2, g2 = 3:5))
...
test_that("find_groups_by_cutoff_val()", {
  expect_equal(find_groups_by_cutoff_val(df_with_ord_factors[, "X"], "d"),
               list(g1 = 1:3, g2 = 4:20))
  expect_equal(find_groups_by_cutoff_val(df_XY_100[, "X"], 25),
               list(g1 = 1:24, g2 = 25:100))
  expect_equal(find_groups_by_cutoff_val(c(20:29, 19:48), 25),
               list(g1 = c(1:5, 11:16), g2 = c(6:10, 17:40)))

  expect_equal(find_groups_by_cutoff_val(c(rep(0, 10), 1), 0),
               list(g1 = c(1:10), g2 = 11))
})

# check find_groups_by_prop -------------------------
test_that("find_groups_by_prop()", {
  expect_equal(find_groups_by_prop(df_with_ord_factors[, "X"], 0.25),
               list(g1 = 1:5, g2 = 6:20))
  expect_equal(find_groups_by_prop(df_XY_100[, "X"], 0.4),
               list(g1 = 1:40, g2 = 41:100))
  expect_equal(find_groups_by_prop(c(20:29, 19:48), 0.1),
  ...
  expect_equal(find_groups_by_prop(c("c", "c", "a", "d", "c"), 0.1,
                                   use_lpSolve = FALSE),
  ...
  expect_equal(find_groups_by_prop(df_with_ord_factors[, "X"], 0.0005,
                                   use_lpSolve = TRUE),
               list(g1 = 1, g2 = 2:20))
  expect_equal(find_groups_by_prop(df_with_ord_factors[, "X"], 0.0005,
                                   use_lpSolve = FALSE),
...
test_that("find_groups_by_values()", {
  expect_equal(find_groups_by_values(df_with_ord_factors[, "X"],
                                     c("a", "b", "e")),
               list(g1 = c(1, 2, 5), g2 = c(3, 4, 6:20)))
  expect_equal(find_groups_by_values(df_XY_100[, "X"],
                                     c(5:10, 25)),
