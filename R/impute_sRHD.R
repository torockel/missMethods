...
#'
#' Impute missing values in a data frame or a matrix
#' using a simple random hot deck
#'
#' There are three types of simple random hot decks implemented.
#' They can be selected via \code{type}:
#' \itemize{
#' \item{"cols_seq" (the default): Each variable (column) is handled separately.
#' If an object (row) has a missing value in a variable (column), then one of the
#' observed values in the same variable is chosen randomly and the missing
#' value is replaced with this chosen value. This is done for all missing
#' values.}
#' \item{"sim_comp": All missing variables (columns) of an object are
#' imputed together ("simultaneous"). For every object with missing values
#' (such an object is called a recipient in hot deck terms),
#' one complete object is chosen randomly and all missing values of the
#' recipient are imputed with the values from the complete object. A complete
#' object used for imputation is called a donor.}
#' \item{"sim_part": All missing variables (columns) of an object are
#' imputed together ("simultaneous"). For every object with missing values
#' (recipient) one donor is chosen. The donor must have observed values in
#' all the variables that are missing in the recipient. The donor
#' is allowed to have unobserved values in the non-missing parts of
#' the recipient. So, in contrast to "sim_comp", the donor can be partly
#' incomplete.}
#' }
#'
#' The parameter \code{donor_limit} controls how often an object can be a
#' donor. This parameter is only implemented for types "cols_seq" and "sim_comp".
#' If \code{type = "sim_part"} and \code{donor_limit} is not \code{Inf}, then
#' an error will be thrown.
#' For "sim_comp" the default value (\code{Inf}) allows every object to be
#' a donor for an infinite number of times (there is no restriction on the
#' times an object can be a donor).
#' If a numeric value less than \code{Inf} is chosen,
#' then every object can be a donor at most
...
#' Setting \code{donor_limit = "min"} chooses automatically the minimum value for
#' \code{donor_limit} that allows imputation of all missing values.
#' For \code{type = "cols_seq"} the donor limit is applied for every column
#' separately.
#'
#'
#' @param ds a data frame or matrix with missing values
#' @param type the type of hot deck; the default ("cols_seq")
#' is a random hot deck that imputes each column separately. Other choices are
#' "sim_comp" and "sim_part". Both impute all missing values in an object (row)
#' simultaneously using a single donor object.
#' The difference between the two types is the choice of objects that can act
#' as donors.
#' "sim_comp:" only completely observed objects can be donors.
#' "sim_part": all objects that have no missing values in the missing parts of
#' a recipient can be donors.
#' @param donor_limit numeric of length one or "min"; how many times an
#' object can be a donor. default is \code{Inf}, so there is no restriction.
#'
...
impute_sRHD <- function(ds, type = "cols_seq", donor_limit = Inf) {
  ...
  if (length(completely_missing_col) != 0) {
    stop("imputation not possible: column(s) ",
...
impute_sRHD_cols_seq <- function(ds, M = is.na(ds), donor_limit) {
  if(donor_limit < min_donor_limit(M, "cols_seq")) {
    stop("donor_limit = ", donor_limit, " is to low! It must be at least ",
...
impute_sRHD_sim_part <- function(ds, M = is.na(ds)) {
  ...
  for(pat_ind in seq_len(nrow(pattern_matrix))) {
    if (any(pattern_matrix[pat_ind, ])) { # if not -> completely observed
      pot_donor_pattern_nrs <- find_pot_donor_pattern_nrs(pattern_matrix,
                                                          pattern_matrix[pat_ind, ])
      if(length(pot_donor_pattern_nrs) == 0L) {
        stop("there is no appropriate donor for the object(s) ",
...
min_donor_limit <- function(M, type) {
  if (type == "sim_comp") {
    ...
    if (nr_comp_obs > 0) {
      ...
      stop("calculation of min donor_limit and Imputation with type = ",
                 type,
