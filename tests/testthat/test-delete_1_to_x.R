...
test_that("delete_MAR_1_to_x() (and delete_1_to_x(), which is called by
  ...
  expect_equal(count_NA(delete_MAR_1_to_x(df_XY_100, 0.001, 3,
                                          miss_cols = "Y", ctrl_cols = "X")),
               c(X = 0, Y = 0))

  # check x -----------------------------------------------
  # x errors
  expect_error(delete_MAR_1_to_x(df_XY_100, 0.2, x = 1:2, "X", "Y"),
               "x must be a single number")
  expect_error(delete_MAR_1_to_x(df_XY_100, 0.2, x = "asdf", "X", "Y"),
               "x must be a single number")
  expect_error(delete_MAR_1_to_x(df_XY_100, 0.2, x = 0, "X", "Y"),
               "x must be greater than 0 and finite")
  expect_error(delete_MAR_1_to_x(df_XY_100, 0.2, x = Inf, "X", "Y"),
               "x must be greater than 0 and finite")
  ...
  expect_warning(df_small_x <- delete_MAR_1_to_x(df_XYZ_100, 0.6, 1e-5, "X", "Y"),
  ...
  expect_warning(df_big_x <- delete_MAR_1_to_x(df_XYZ_100, 0.6, 1e10, "X", "Y"),
  ...
  expect_warning(df_miss <- delete_MAR_1_to_x(df_XY_100, 0.9, 3, "X", "Y"),
  ...
  expect_warning(df_miss <- delete_MAR_1_to_x(df_XY_100, 0.9, 0.1, "X", "Y"),
  ...
  test_max_x <- suppressWarnings(replicate(1000,
                    sum(is.na(delete_MAR_1_to_x(df_XY_20, 0.9, 3, "X", "Y",
                                                stochastic = TRUE)[1:10, "X"]))))
  expect_true(mean(test_max_x) < 9 && mean(test_max_x) > 7)

  # check special ctrl_col cases --------------------------
  # ctrl_col constant
  expect_warning(df_miss <- delete_MAR_1_to_x(df_XY_X_constant, 0.2, 3,
                                          miss_cols = "Y", ctrl_cols = "X"),
                 "is constant")
  expect_equal(count_NA(df_miss),c(X = 0, Y = 4))
  # ctr_col nearly constant
  expect_equal(count_NA(delete_MAR_1_to_x(df_XY_X_one_outlier, 0.2, 3,
                                         miss_cols = "Y", ctrl_cols = "X")),
               c(X = 0, Y = 4))
  ...
  expect_equal(attr(delete_MAR_1_to_x(df_XYZ_100, 0.2, 4, "X", "Y",
                                          add_realized_x = TRUE),
                        "realized_x", exact = TRUE),
                   c(X = 4))
  ...
  expect_equal(attr(delete_MAR_1_to_x(df_XYZ_100, 0.3, 3, "X", "Y",
                                     add_realized_x = TRUE),
                        "realized_x", exact = TRUE),
                   c(X = 23/7))
...
test_that("delete_MNAR_1_to_x", {
  ...
  expect_equal(count_NA(delete_MNAR_1_to_x(df_XYZ_100, 0.1, 3, c("X", "Z"))),
               c("X" = 10, "Y" = 0, "Z" = 10))
  expect_equal(attr(delete_MNAR_1_to_x(df_XYZ_100, 0.1, 3.5, c("X", "Z"),
                                       add_realized_x = TRUE),
                    "realized_x", exact = TRUE),
               c(X = 4, Z = 4))
